...
int32 AAIO_Group::PullItem(TSubclassOf<class UFGItemDescriptor> Item, const int32 Count,
                           const bool bSkipOutputs) {
	...
	while (RemainingCount > 0 && --i >= 0) {
		...
		if (Provider == nullptr || (bSkipOutputs && Provider->IsOutput())) {
			continue;
		}
